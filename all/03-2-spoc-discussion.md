...
### 6.3	页式存储管理
 ...
 页是一段地址空间，帧是页中一个地址，多个页构成页表，快表是虚拟地址转实地址时提供快速查询用，Cache是为了解决CPU和存储器间读写速度的差异，把內存中的一部分装进去的硬件。
...
### 6.4	页表概述
 ...
 页內偏移，虚页号，页內偏移提供实页表中对应偏移量，操作系统需要虚页号从TLB中查找对应的实页。
...
### 6.5	快表和多级页表
 ...
 TLB是虚地址到实地址转换时用的，Cache是CPU和存储器间的为了解决存储速度差异的。
 
 1. 为什么快表中查找物理地址的速度非常快？它是如何实现的？为什么它的的容量很小？
 
 快表TLB是硬件实现的，因此它可以很快找到对应的实地址。
 
 1. 什么是多级页表？多级页表中的地址转换流程是什么？多级页表有什么好处和麻烦？
 
 多级页表是由多组页表构成，从一开始的虚地址经多级转换至最终物理地址。好处是使每一级页表的容量较小，麻烦是转换过程较复杂。
